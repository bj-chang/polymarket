#!/usr/bin/env python3
# btc_updown_gui_multi_theos_mu_no_px_with_sigma_chart_and_price_chart__BACKFILL_10M_PER_SECOND__15M_or_1H.py
# Window selectable: 15m or 1h (default 15m).
# - Strike = open of the chosen window (15m or 1h)
# - Rollover matches the chosen window length
# - Progress/time-to-close reflect the selected window
# - Backfill button still builds per-second series for last 10 minutes
# FIX: σ (volatility) chart now throttled to ~1 Hz so it retains ~10 minutes of data.
...
def get_kline_open_close(start_et: datetime, end_et: datetime, mode: str):
    """Return open, close, open_ms, close_ms for the window (15m or 1h) covering [start_et, end_et)."""
    interval = "15m" if mode == "15m" else "1h"
...
def current_et_quarter_window(now_utc: datetime):
    now_et = now_utc.astimezone(ET)
...
def calc_window(now_utc: datetime, mode: str):
    return current_et_quarter_window(now_utc) if mode == "15m" else current_et_hour_window(now_utc)
...
# ------------------- Worker thread -------------------
def worker(out_q: "queue.Queue[dict]", ctrl_q: "queue.Queue[dict]", stop_event: threading.Event, mode_state: dict):
    def window_secs_for(mode: str) -> float:
        return 15 * 60.0 if mode == "15m" else 60 * 60.0

    # Initialize window/strike
    mode = mode_state["mode"]
    now = datetime.now(tz=UTC)
    start_et, end_et = calc_window(now, mode)
    start_utc = start_et.astimezone(UTC); end_utc = end_et.astimezone(UTC)

    strike = None
    while not stop_event.is_set() and strike is None:
        try:
            k = get_kline_open_close(start_et, end_et, mode)
            if k: strike = k[0]
        except Exception:
            pass
        time.sleep(0.3)
    ...
    while not stop_event.is_set():
        # Handle control commands (mode switch)
        try:
            while True:
        ...
        # Rollover on window end
        if now >= end_utc:
            try:
                _, final_close, _, _ = get_kline_open_close(start_et, end_et, mode)
                ...
                out_q.put({"event": "status", "msg": f"Closed {mode}: couldn't fetch final close ({e}). Rolling…"})
            start_et, end_et = calc_window(now, mode)
            start_utc = start_et.astimezone(UTC); end_utc = end_et.astimezone(UTC)
            strike = None
            while not stop_event.is_set() and strike is None:
        ...
        if px is not None and strike is not None:
            ...
            frac_elapsed = min(1.0, max(0.0, elapsed / (900.0 if mode == "15m" else 3600.0)))
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("BTCUSDT — ET Window (15m / 1h) | Δ Price vs Strike & Vol (per-second backfill)")
        ...
        # Shared state & queues
        self.mode_state = {"mode": "15m"}  # default
        self.q = queue.Queue()       # worker -> UI
        self.ctrl_q = queue.Queue()  # UI -> worker
        self.stop_event = threading.Event()
        self.worker_thread = threading.Thread(target=worker, args=(self.q, self.ctrl_q, self.stop_event, self.mode_state), daemon=True)
        ...
        ttk.Label(right, text="Window:", style="Caption.TLabel").grid(row=0, column=0, padx=(0,6))
        self.mode_var = tk.StringVar(value=self.mode_state["mode"])
        self.cmb_mode = ttk.Combobox(right, values=["15m","1h"], textvariable=self.mode_var, state="readonly", width=6)
        self.cmb_mode.grid(row=0, column=1)
        ...
        self.var_strike_caption = tk.StringVar(value="Strike (15m open):")
    ...
    def _init_price_chart(self, parent):
        ...
        self.ax_px.yaxis.set_major_locator(mticker.MultipleLocator(10))
    ...
    def on_backfill_10m_persec(self):
        ...
        """
        try:
            # Ensure strike (for current mode)
            if self.last_strike is None:
                now_utc = datetime.now(tz=UTC)
                start_et, end_et = calc_window(now_utc, self.mode_state["mode"])
                k = get_kline_open_close(start_et, end_et, self.mode_state["mode"])
                if not k:
    ...
    def on_mode_change(self, evt=None):
        ...
        self.var_strike_caption.set(f"Strike ({'15m open' if new_mode=='15m' else 'hour open'}):")
        self.last_strike = None
        self.ctrl_q.put({"type": "set_mode", "mode": new_mode})
        self.var_status.set(f"Switched to {new_mode} window.")
    ...
    def handle(self, msg: dict):
        ...
        if evt == "tick":
            ...
            mode = msg.get("mode", self.mode_state["mode"])
            self.lbl_window.config(
                text=f"ET window ({mode}): {start_et.strftime('%Y-%m-%d %H:%M')} → {end_et.strftime('%H:%M')}  "
