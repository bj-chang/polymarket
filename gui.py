#!/usr/bin/env python3
# btc_updown_gui_multi_theos_mu_no_px_with_sigma_chart_and_price_chart__BACKFILL_10M_PER_SECOND.py
# Backfill button fetches per-second data from aggTrades (no interpolation) and computes per-second volatility
# (rolling 60s std of 1s log returns, annualized). Price chart Y = Δ vs strike with signed 10-unit ticks.
...
def fetch_agg_trades_range(start_ms: int, end_ms: int, limit: int = 1000):
    ...
    while True:
        ...
        # Move cursor forward to avoid duplicates; stop if we're at/near the end
        next_ms = last_T + 1
        if next_ms >= end_ms:
            break
        cursor_ms = next_ms
        # Safety: if fewer than ~100 trades came back and time hasn't advanced much, break to avoid loops
...
def worker(out_q: "queue.Queue[dict]", stop_event: threading.Event):
    ...
    while not stop_event.is_set():
        ...
        if tnow - last_mu_t >= REFRESH_MU:
            try:
                mu_ann = mu_ewma_ann_from_cache(cache_1m)
            except Exception:
                pass
            last_mu_t = tnow
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("BTCUSDT — Current ET Hour | Δ Price vs Strike & Vol (backfill: per-second)")
    ...
    def _init_price_chart(self, parent):
        ...
        # Reference at Δ=0 (strike) — may be off-screen depending on limits
    ...
    def on_backfill_10m_persec(self):
        ...
        try:
            ...
            # Map second -> last price seen in that second (UTC seconds)
            sec_to_price = {}
            for t in trades:
                sec = int(int(t["T"]) // 1000)
                price = float(t["p"])
                sec_to_price[sec] = price  # overwrite to keep the last within the second

            # Build per-second series for EXACT last 10 minutes
            end_sec = int(end_utc.timestamp())
            start_sec = end_sec - 600  # 600 seconds back
            times_utc = list(range(start_sec, end_sec + 1))
            prices = []
            last_price = None
            # For robustness, if the first seconds have no trade, try to find earliest price in our map
            fallback_price = None
            ...
            for s in times_utc:
                if s in sec_to_price:
                    ...
                    # If still None, use fallback (first known recent price)
                    last_price = fallback_price
                # If still None (extremely unlikely for BTC), skip this second
                if last_price is None:
                    continue
                prices.append(last_price)

            # Align times to the number of prices kept (in case of initial skips)
            ...
            for i in range(len(prices)):
                if i < ROLLING_SEC_FOR_VOL:
                    # not enough history: repeat first available sigma once we have it
                    sigmas.append(sigmas[-1] if sigmas else SIGMA_MIN)
                    continue
                window = rets_1s[i-ROLLING_SEC_FOR_VOL:i]  # last 60 1s returns
                s = ann_sigma_from_returns(window, SECS_PER_YEAR)
                s = min(SIGMA_MAX, max(SIGMA_MIN, s)) if s > 0 else SIGMA_MIN
                sigmas.append(s)
    ...
    def handle(self, msg: dict):
        ...
        if evt == "tick":
            ...
            # Live appends (price & the original 1m-σ series for the table/line)
            ...
            self._push_sigma_point(now_et, sigma_1m_current)  # this line drives the chart live; after backfill it's fine
            self._redraw_sigma_chart()
